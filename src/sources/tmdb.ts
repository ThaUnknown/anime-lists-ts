...
export async function appendMissingIds (itemList: AnimeItem[]): Promise<void> {
  ...
  for (const item of itemList) {
    ...
    // Only merge if type is 'TV' or 'MOVIE' (like Java logic)
    ...
    if (hasTmdbId && type) {
      ...
      // else: skip merge for other types
    } else if (!hasTmdbId) {
      // If no TMDB id, allow merge attempt (for fallback by tvdb/imdb)
      shouldMerge = true
    }
    ...
    if (mergeItem) {
      mergeItem.themoviedb_id ??= item.themoviedb_id
      mergeItem.tvdb_id ??= item.tvdb_id
      mergeItem.imdb_id ??= item.imdb_id
    }
  }
}
