...
import type { NBTItemData } from '../caches/nbt'
...
export class AnimeItem {
  ...
  static fromNBTData (data: NBTItemData): AnimeItem {
    const animeItem = new AnimeItem()
    animeItem.themoviedb_id = data.tmdbId === 0 ? undefined : data.tmdbId
    animeItem.imdb_id = data.imdbId
    animeItem.tvdb_id = data.tvdbId
    return animeItem
  }
  ...
  static fromAnimeListsSource (item: AnimeListsItem): AnimeItem {
    ...
    if (tmdbSeason != null && tmdbSeason !== '0') {
      season ??= {}
      season.tmdb = this.parseStringToInteger(item.anidbid, 'tmdb season', tmdbSeason)
    }
    ...
    if (
      ...
      season ??= {}
      season.tvdb = this.parseStringToInteger(item.anidbid, 'tvdb season', defaultTvdbSeason)
    }
